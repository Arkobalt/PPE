...
$code_reponse = 406;
    ...
    {	// récupération de la liste des utilisateurs à l'aide de la méthode getTousLesUtilisateurs de la classe DAO
        ...
        if ($nbReponses == 0) {
            ...
            $msg = $nbReponses . " autorisation(s) accordée(s) " . $pseudo. ".";
            $code_reponse = 200;
        }
    }
    }
...
function creerFluxXML($msg, $lesUtilisateursAutorisant)
{
    ...
    if (sizeof($lesUtilisateursAutorisant) > 0) {
        ...
        foreach ($lesUtilisateursAutorisant as $unUtilisateurAutorisant)
        {
            ...
            $elt_id         = $doc->createElement('id', $unUtilisateurAutorisant->getId());
            $elt_utilisateur->appendChild($elt_id);
            
            $elt_pseudo     = $doc->createElement('pseudo', $unUtilisateurAutorisant->getPseudo());
            $elt_utilisateur->appendChild($elt_pseudo);
            
            $elt_adrMail    = $doc->createElement('adrMail', $unUtilisateurAutorisant->getAdrMail());
            $elt_utilisateur->appendChild($elt_adrMail);
            
            $elt_numTel     = $doc->createElement('numTel', $unUtilisateurAutorisant->getNumTel());
            $elt_utilisateur->appendChild($elt_numTel);
            
            $elt_niveau     = $doc->createElement('niveau', $unUtilisateurAutorisant->getNiveau());
            $elt_utilisateur->appendChild($elt_niveau);
            
            $elt_dateCreation = $doc->createElement('dateCreation', $unUtilisateurAutorisant->getDateCreation());
            $elt_utilisateur->appendChild($elt_dateCreation);
            
            $elt_nbTraces   = $doc->createElement('nbTraces', $unUtilisateurAutorisant->getNbTraces());
            $elt_utilisateur->appendChild($elt_nbTraces);
            
            if ($unUtilisateurAutorisant->getNbTraces() > 0)
            {   $elt_dateDerniereTrace = $doc->createElement('dateDerniereTrace', $unUtilisateurAutorisant->getDateDerniereTrace());
...
function creerFluxJSON($msg, $lesUtilisateursAutorisant)
{
    ...
    if (sizeof($lesUtilisateursAutorisant) == 0) {
        ...
        {	// crée une ligne dans le tableau
            ...
            $unObjetUtilisateur["id"] = $unUtilisateurAutorisant->getId();
            $unObjetUtilisateur["pseudo"] = $unUtilisateurAutorisant->getPseudo();
            $unObjetUtilisateur["adrMail"] = $unUtilisateurAutorisant->getAdrMail();
            $unObjetUtilisateur["numTel"] = $unUtilisateurAutorisant->getNumTel();
            $unObjetUtilisateur["niveau"] = $unUtilisateurAutorisant->getNiveau();
            $unObjetUtilisateur["dateCreation"] = $unUtilisateurAutorisant->getDateCreation();
            $unObjetUtilisateur["nbTraces"] = $unUtilisateurAutorisant->getNbTraces();
            if ($unUtilisateurAutorisant->getNbTraces() > 0)
            {   $unObjetUtilisateur["dateDerniereTrace"] = $unUtilisateurAutorisant->getDateDerniereTrace();
            }
